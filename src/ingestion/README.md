...
| `build_index.py` | `products.parquet` | `data/chroma_db/` | Embeds `title + features` with `all-MiniLM-L6-v2`, upserts into a persistent Chroma collection (cosine distance) with filterable metadata. `ingredients` is 100% empty in this dataset (see [Known data-quality limitations](#known-data-quality-limitations)), so it's excluded from the embedding text — it contributes nothing — but kept in metadata |
...
`price_per_unit` and `unit` are both indexed as Chroma metadata and both returned by `RagRetriever.search`, deliberately as a pair: a normalized price is uninterpretable without knowing whether it's per ounce or per count, and two per-unit prices are only comparable when their units match. Downstream, the Answerer uses them for value comparisons ("cheapest", "best deal") and the UI renders them as a "Unit price" column. See the `price_per_unit` entry under [Known data-quality limitations](#known-data-quality-limitations) for why a minority of rows carry an implausible figure, and why they're surfaced rather than filtered.
...
- **`price_per_unit` is only derived when a quantity+unit** (oz, lb, ct, etc.) is parseable from the title/weight/description text — 8,838 of 10,002 rows (88%). `price` itself is populated for 9,839/10,002 (98%). Units are overwhelmingly weight-based: 4,961 `oz`, 3,656 `lb`, then 304 `ct` and a long tail of `ml`/`l`/`gal`. Since a per-unit price is only comparable against another in the *same* unit, that split matters — an `oz` figure and a `ct` figure sitting in the same result set must not be compared, and the Answerer's prompt says so explicitly.
- **`price_per_unit` is unreliable wherever `Shipping Weight` is a placeholder.** Values like `0.16 ounces` and `0.32 ounces` recur across hundreds of unrelated products (282 rows share just those two), and `1.00 lb` behaves the same way — clearly a default weight tier in the raw scrape, not a measurement. Combined with a real price this yields nonsense: a $802 stool at "$5,015/oz". 62 rows (0.7% of those with a per-unit price) exceed $100/unit, but a threshold doesn't cleanly separate good from bad — the same implausible pattern continues below it (a $99.99 bed skirt at "1.00 lb" → $99.99/lb). So no cutoff is applied: the value is surfaced as-is with its unit, the Answerer is instructed to omit obviously-absurd figures rather than repeat them, and this limitation is stated in the presentation. Filtering behind an arbitrary number would hide the issue without actually fixing it.
